...
// LIVELLO 2 CARBONIO TRE DISTRATTORI

import Foundation
import GameplayKit
import AVFoundation

class LivelloCarbonio: SKScene,SKPhysicsContactDelegate {
 private var ampolla = Ampolla()
    ...
    override func didMove(to view: SKView) {
        ...
        ElementText.text = "CARBON"
        ElementText.position = CGPoint(x: 325, y: 830)
    ...
    override func update(_ currentTime: CFTimeInterval) {
       ...
       private func createRandomBlock() {
           let randomBlock = arc4random_uniform(10) > 3 ? Carbonio() : Ferro()
        randomBlock.size = CGSize(width: 65, height: 65)
        randomBlock.zPosition = 1
        let otherrandomblock = arc4random_uniform(8) > 3 ? Cloro() : Magnesio()
...
func didBegin(_ contact: SKPhysicsContact) {
       ...
       if let flag = otherNode as?  Carbonio {
        flag.removeFromParent()
