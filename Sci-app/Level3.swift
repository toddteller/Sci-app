...
//  Copyright © 2019 Hangover. All rights reserved.
//
...
class Level3: SKScene,SKPhysicsContactDelegate {
    ...
    let cameraNode = SKCameraNode() //Camera
          ...
          shape3.alpha = 0
           ...
           playButton = self.childNode(withName: "startButton") as? SKSpriteNode
            ...
            run(SKAction.repeatForever(SKAction.sequence([SKAction.run {
               ...
               },SKAction.wait(forDuration: 4.5),SKAction.run{
                   [weak self] in self?.setAstro()} ])))
           run(SKAction.repeatForever(SKAction.sequence([SKAction.run {
                 [weak self] in self?.spawnCollectibles()
           },SKAction.wait(forDuration: 4.0),SKAction.run{
               [weak self] in self?.setAstro()}])))
           
           run(SKAction.repeatForever(SKAction.sequence([SKAction.run{
               [weak self] in self?.spawnMeteor()},SKAction.wait(forDuration: 10.0)])))
           
           //-----------------------------c o l l i s i o n ----------------------------------------//
           
           astro.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: 70, height: 70))
       ...
       func create_Bg() -> SKSpriteNode { //Create a unique bg formed by two parts
           let l1_bg1 = SKSpriteNode(imageNamed: "3-planet-background1") //Part one
           ...
           //------------------------------------------------------------------------------------------//
           let l1_bg2 = SKSpriteNode(imageNamed: "3-planet-background2") // Part two
           ...
           final_bg.addChild(l1_bg2) //Add part two
           return final_bg
       }
       ...
       func createFullAstronaut() -> SKAction{
        ...
        func roll() -> SKAction{
            ...
            var rolltime = SKAction.rotate(toAngle: 1080, duration: 2.5)
            let astro_animation = SKAction.group([SKAction.animate(with: astro_texture, timePerFrame: 2.7),rolltime])
               ...
               if( touches.count == 1) {
                      if  astro.position.y<200{
                       let up = SKAction.moveTo(y: astro.position.y+200,duration: 0.5)
                        let down = SKAction.moveTo(y: astro.position.y, duration: 2.0)
       ...
       func spawnRock()  {
           if(numb>0 && numb_score<10) {
                  ...
                  let rock : SKSpriteNode = SKSpriteNode (imageNamed: "3-planet-rocks\(random)")
                  rock.size = CGSize(width: 175, height: 83)
       ...
       func setAstro(){
         let actions = SKAction.run {
           ...
           run(SKAction.sequence([SKAction.wait(forDuration: 1.5),actions]))
       }
       ...
       func spawnCollectibles(){
           if(numb>0 && numb_score<10) {
               ...
               let collectible : SKSpriteNode = SKSpriteNode(imageNamed: "money3-1")
       ...
       func score(numb_score: String) {
           ...
           label.text = "Score : \(numb_score)/10"
           label.fontSize = 40
           label.zPosition = 3
       ...
       func spawnMeteor() {
           if (numb>0 && numb_score<10) {
           let random = CGFloat.random(min:200,max:astro.position.y+180)
       ...
       func createFullCoin() -> SKAction {
           ...
           for i in 1...4{
               coin_texture.append(SKTexture(imageNamed: "money3-\(i)"))
           }
