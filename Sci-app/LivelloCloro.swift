...
class LivelloCloro: SKScene,SKPhysicsContactDelegate {
 private var ampolla = Ampolla()
    ...
    override func update(_ currentTime: CFTimeInterval) {
       ...
       private func createRandomBlock() {
           let randomBlock = arc4random_uniform(10) > 3 ? Cloro() : Ossigeno()
        randomBlock.size = CGSize(width: 65, height: 65)
        randomBlock.zPosition = 1
        let otherrandomblock = arc4random_uniform(8) > 3 ? Magnesio() : Azoto()
...
func didBegin(_ contact: SKPhysicsContact) {
       ...
       if let flag = otherNode as?  Cloro {
        flag.removeFromParent()
